...
kde_bounded_2 <- function(thetas, weights, bounds, bins, h=NULL) {
  ...
  unscale <- function(x) {
    x * (bounds$max - bounds$min) + bounds$min
  }
  ...
  if(is.null(h)){
    ...
    h_u <- h / (bounds$max - bounds$min)
  }
  
  grid_min_u <- qbeta(0.01, thetas_min_u / h_u + 1, (1 - thetas_min_u) / h_u + 1)
  grid_max_u <- qbeta(0.99, thetas_max_u / h_u + 1, (1 - thetas_max_u) / h_u + 1)

  grid_def_u <- tibble(from=as_vector(grid_min_u),
                       to=as_vector(grid_max_u),
                       by=(to - from) / bins)
  
  grid_borders_u <- pmap(grid_def_u, seq)
  ...
  long_h_u <- tibble(variable = names(thetas), h_value = h_u)
  ...
  grid_def <- grid_def_u %>% transmute(from=unscale(from), to=unscale(to), by=(to - from) / bins)
  evalp <- unscale(evalp_u)
  half_grid_def <- grid_def$by / 2
  limit_below <- evalp - grid_def$from
  limit_above <- grid_def$to - evalp
  cond_below <- (half_grid_def < limit_below)
  cond_above <- (half_grid_def < limit_above)
  length_below <- (half_grid_def) * cond_below + limit_below * (!cond_below)
  length_above <- (half_grid_def) * cond_above + limit_above * (!cond_above)
  ...
  fhat <- evalp %>% mutate(density = fhat_u$density / prod(bounds$max - bounds$min), vcell = vcell)

  list(result=fhat, 
       grid_def=grid_def, 
       grid_borders=pmap(grid_def, seq),
       dim=ncol(thetas),
       h=h_u * (bounds$max - bounds$min))
}
